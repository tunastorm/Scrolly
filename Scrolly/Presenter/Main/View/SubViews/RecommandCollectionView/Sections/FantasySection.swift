...
enum FantasySection: String, MainSection {
    ...
    var query: HashTagsQuery {
        return switch self {
        case .banner, .popular:
            HashTagsQuery(next: nil, limit: "10", productId: APIConstants.ProductId.novelInfo, hashTag: APIConstants.SearchKeyword.fantasy)
        case .newWaitingFree:
            HashTagsQuery(next: nil, limit: "6", productId: APIConstants.ProductId.novelInfo, hashTag: APIConstants.SearchKeyword.fantasy)
        case .recently:
            HashTagsQuery(next: nil, limit: "10", productId: APIConstants.ProductId.novelEpisode, hashTag: APIConstants.SearchKeyword.fantasy)
        }
    }
    
    func convertData(_ section: FantasySection, _ model: [PostsModel]) -> [PostsModel] {
        switch section {
        case .banner:
